...
<xsl:template name="example">
	...
	<xsl:variable name="columns" select="5"></xsl:variable>
	<xsl:for-each select="interlinear/lineGroup"> 
				...
				<xsl:for-each select="line"> 
					<row>
						<xsl:for-each select="wrd/langData">
							<xsl:text disable-output-escaping="yes">&lt;entry lang="</xsl:text>
